...
void SimulationEngineSingleThread::reserve_observations(std::vector<std::vector<Observation>> &observations,
    ...
    for (int i = 0; i <= edc.stc.last_alive_position; i++) {
        ...
        if (organism.anatomy.c.data[(int)BlockTypes::EyeBlock-1] > 0 && organism.anatomy.c.data[(int)BlockTypes::MoverBlock-1] > 0 && organism.move_counter == 0) {
...
void SimulationEngineSingleThread::get_observations(EngineDataContainer &edc, SimulationParameters &sp,
                                                    ...
                                                    {
    if (organism.anatomy.c.data[(int)BlockTypes::EyeBlock] <= 0 || organism.anatomy.c.data[(int)BlockTypes::MoverBlock-1] <= 0) {return;}
    if (organism.move_counter != 0) {return;}

    for (int eye_i = 0; eye_i < organism.anatomy.c.data[(int)BlockTypes::EyeBlock-1]; eye_i++) {
...
bool SimulationEngineSingleThread::calculate_continuous_move(EngineDataContainer &edc, Organism &organism,
    ...
    cd.p_vx += (cd.p_fx - cd.p_vx * sp.continuous_movement_drag) / mass * organism.anatomy.c.data[int(BlockTypes::MoverBlock)-1];
    cd.p_vy += (cd.p_fy - cd.p_vy * sp.continuous_movement_drag) / mass * organism.anatomy.c.data[int(BlockTypes::MoverBlock)-1];
...
void SimulationEngineSingleThread::make_decision(EngineDataContainer &edc, SimulationParameters &sp, Organism &organism, lehmer64 &gen) {
    bool moved = false;
    if (organism.anatomy.c.data[(int)BlockTypes::MoverBlock-1] <= 0) {return;}
